...
var bridge = {
  ...
  call: function call(functionName, args, callback) {
    ...
    "function" == typeof args && (callback = args, args = {}); //参数对象 {data: args/null}
    // void 0 解释 https://stackoverflow.com/questions/7452341/what-does-void-0-mean

    args = {
      data: void 0 === args ? null : args
    }; // 如果是异步并且要接收返回值的

    if ("function" == typeof callback) {
      // callback标识,计数器
      var tag = "dscb" + window.dscb++; // 保存回调   window[dscbX] = callback Function

      window[tag] = callback; // args = {data: args/null,_dscbstud:tag}
    ...
    var ret = ""; // 这里不会走到,目前从源里没有看到有什么地方注入了_dsbridge
  ...
  unregister: function unregister(jsFunctionName) {
  ...
  register: function register(jsFunctionName, receiveParamsFunction, callback) {
    // 判断是同步还是异步  callback = { _obs:{} };
    callback = callback ? window._dsaf : window._dsf; //只执行一次,为了调用一次native的dsinit方法

    window._dsInit || (window._dsInit = !0, setTimeout(function () {
      bridge.call("_dsb.dsinit");
    }, 0)); //将call保存在对应的数据结构中, callback = { jsFunctionName : receiveParamsFunction: , _obs:{} };

    "object" == _typeof(receiveParamsFunction) ? callback._obs[jsFunctionName] = receiveParamsFunction : callback[jsFunctionName] = receiveParamsFunction;
  },
  // js注册供Native调用的异步方法
...
(function () {
  ...
  if (window._dsf) return; // 2. Window添加一个 `_dsf` 对象属性, 保存同步方法
  ...
  window["dscb"] = 0; // 5. Window添加一个 bridge属性

  window["dsBridge"] = bridge; //6. Window添加一个界面关闭方法
